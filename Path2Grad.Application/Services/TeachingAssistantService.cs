...
{
    public class TeachingAssistantService : ITeachingAssistantService
    {
        ...
        {
            var assistant = await _supervisorRepo.GetByEmailAsync(email);
            if (assistant == null) return Enumerable.Empty<object>();

            var projectIds = await _supervisorProjectRepo.GetProjectIdsBySupervisorIdAsync(assistant.SupervisorId);
            ...
            return filteredProjects.Select(p => new
            {
                ...
                Doctors = p.SupervisorProjects
                    .Where(sp => sp.Supervisor.Position == "Doctor")
                    .Select(sp => new
                    {
        ...
        {
            var doctor = await _supervisorRepo.GetByEmailAsync(email);
            if (doctor == null) return "Teaching Assistant not found";
